...
We know that the best situation for a "best-of-seven" is having one team winning the first four games.

Then, we assume that Braves will have its last win at the fifth game, then it will need to win any three of the previous four games; in other words, it will need to lose one of the previous four games.

If we continue this way, putting the last win at the sixth or seventh game, then Braves will need to lose two or three of the previous games.

Sum the probability of these four situations occuring, then we could get the answer.
...
Here, we first load the package that we need.
...
From the result, we could know that the probability for Braves to win a best-of-7 is 0.608 when givin the prob of Braves to win is 0.55.
...
According to this new vector that containing all the probability of Braves winning the World Series (given PB=0.55) under different series length, we can see that when they have seventy-one games (best-of-71), Braves could win the world series with a probability equal or greater than 0.8.
```{r}
...
By calculating, we could get P(PB=0.55|Braves win World Series in 7 games) under the assumption that PB=0.55 is 0.5, and P(PB=0.55|Braves win World Series in 7 games) under the assumption that PB=0.45 is 0.776.

```{r}
# When PB = 0.55
P <- (pnbinom(3,4,0.55)*(1/2))/pnbinom(3,4,0.55)
P
# When PB = 0.45
P_a <- (pnbinom(3,4,0.55)*(1/2))/pnbinom(3,4,0.45)
P_a
```
