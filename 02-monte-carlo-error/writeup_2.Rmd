...
* Large-scale computer-based simulations is a method of producing quantitative and
predictive information. It is better that simulation results can come with a high confidence level.
...
* Since each time simulation are not the same, simulations involve their attendant errors, and these error models can be used to improve the prediction.
...
-------------------------------------------
Here we perform a 14 X 5 factorial experiment simulation that estimates the error for each combination of replicate number (22, 23, …, 215) and probability (0.01, 0.05, 0.10, 0.25, 0.50). Let p hat denote the probability estimated from simulation, and let p denote the true underlying probability.
...
From the top of the graph to the bottom: the line that made based on each probability are following the order of 0.50, 0.25, 0.10, 0.05, and 0.01. We can see that absolute error is deeply decreasing when the replicate number (in log2 scale) getting larger, with highest relative error laying on p=0.50.
...
From the bottom of the graph to the top: the line that made based on each probability are following the order of 0.50, 0.25, 0.10, 0.05, and 0.01. We can see that relative error is deeply decreasing when the replicate number (in log2 scale) getting larger, with highest relative error laying on p=0.01.
...
Then we try to generate the results with the y-axis is on the log10 scale and find a relationship between using log10 scale and log2 scale.
...
We could see that they have a linear relationship for both absolute and relative error.
