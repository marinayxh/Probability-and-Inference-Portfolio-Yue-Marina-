...
M <- 5000 # studies
N <- 201 # sample size
b_a <- array(rlnorm(M*N, 0, 1), c(M, N))
b_b <- b_a %>% apply(1, function(x){sort(x)[50]})
hist(b_b, breaks = 100, freq = FALSE, main = "")
curve(fk_6c(x, 50, 201), lwd = 3, add = TRUE, col = "blue")
```
...
plot(ecdf(b_b))
curve(Fk(x, 50, 201), lwd = 3, add = TRUE, col = "blue")
```
...
out_201 <- rep(NA)
for (i in 1:R){
  sample_201 = rlnorm(201,meanlog = 0, sdlog = 1)
  out_201[i] = quantile(sample_201,probs = 0.25)
}


out_301 <- rep(NA)
for (i in 1:R){
  sample_301 = rlnorm(301,meanlog = 0, sdlog = 1)
  out_301[i] = quantile(sample_301,probs = 0.25)
}


out_401 <- rep(NA)
for (i in 1:R){
  sample_401 = rlnorm(401,meanlog = 0, sdlog = 1)
  out_401[i] = quantile(sample_401,probs = 0.25)
}



## try function here
a_6_function <- function(n){
  output_6a <- rep(NA)
  for (i in 1:1000){
    sample_6a = rlnorm(n,0, 1)
    output_6a[i] = quantile(sample_6a,probs = 0.25)
  }
  return(density(output_6a))
}

plot(a_6_function(201), ylim = c(0,12), col = 4)
lines(a_6_function(301), col = 3)
lines(a_6_function(401), col = 2)
